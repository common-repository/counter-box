...
class ImporterExporter {
	...
	public static function form_export(): void {
        ...
        <form method="post">
            ...
            <p>
				<?php
				submit_button( __( 'Export All Data', 'counter-box' ), 'secondary', 'submit', false ); ?><?php
        ...
        <form method="post" enctype="multipart/form-data" action="">
            ...
            <p>
                <label>
					...
					<?php esc_html_e( 'Update item if item already exists.', 'counter-box' ); ?>
                </label>
            ...
            <p>
				<?php
				submit_button( __( 'Import', 'counter-box' ), 'secondary', 'submit', false ); ?><?php
	...
	public static function import_data(): void {
		...
		if ( ! isset( $_FILES['import_file'] ) || empty( $_FILES['import_file']['name'] ) ) {
			wp_die( esc_attr__( 'Please select a file to import', 'counter-box' ),
				esc_attr__( 'Error', 'counter-box' ),
				[ 'response' => 400 ] );
		}

        if ( self::get_file_extension( sanitize_text_field( $_FILES['import_file']['name'] ) ) !== 'json' ) {
			wp_die(
				esc_html__( 'Please upload a valid .json file', 'counter-box' ),
				esc_html__( 'Error', 'counter-box' ),
				[ 'response' => 400 ] );
		}
